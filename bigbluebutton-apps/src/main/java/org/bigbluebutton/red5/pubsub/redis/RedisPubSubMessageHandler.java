...
//		System.out.println("in red5 getting message: " + channel + " " + message);
		if (channel.equalsIgnoreCase(ChannelConstants.FROM_CHAT_CHANNEL)) {
			chatMessageSender.handleChatMessage(message);
		} else if (channel.equalsIgnoreCase(ChannelConstants.FROM_PRESENTATION_CHANNEL)) {
			presentationMessageSender.handlePresentationMessage(message);
		} else if (channel.equalsIgnoreCase(ChannelConstants.FROM_MEETING_CHANNEL)) {
			meetingMessageSender.handleMeetingMessage(message);
		} else if (channel.equalsIgnoreCase(ChannelConstants.FROM_USERS_CHANNEL)) {
			userMessageSender.handleUsersMessage(message);
		} else if (channel.equalsIgnoreCase(ChannelConstants.FROM_WHITEBOARD_CHANNEL)) {
			whiteboardMessageSender.handleWhiteboardMessage(message);
		} else if (channel.equalsIgnoreCase(ChannelConstants.FROM_SYSTEM_CHANNEL)) {
			bbbAppsIsKeepAliveHandler.handleKeepAliveMessage(message);
		} else if (channel.equalsIgnoreCase(ChannelConstants.FROM_DESK_SHARE_CHANNEL)) {
			deskShareMessageSender.handleDeskShareMessage(message);
		} else if (channel.equalsIgnoreCase(ChannelConstants.FROM_POLLING_CHANNEL)) {
			pollingMessageSender.handlePollMessage(message);
		} else if (channel.equalsIgnoreCase(ChannelConstants.FROM_CAPTION_CHANNEL)) {
