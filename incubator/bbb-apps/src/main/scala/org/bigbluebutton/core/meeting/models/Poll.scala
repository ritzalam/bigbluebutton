...
import org.bigbluebutton.core.domain
...
class Poll(val id: String, val questions: Array[Question], val numRespondents: Int, val title: Option[String]) {
  ...
  def respondToQuestion(questionID: Int, responseID: Int, responder: domain.Responder) {
    questions.foreach(q => {
      if (q.id == questionID) {
...
class Question(val id: Int, val questionType: String, val multiResponse: Boolean, val text: Option[String], val answers: Array[Answer]) {
  ...
  def respondToQuestion(id: Int, responder: domain.Responder) {
    answers.foreach(r => {
...
class Answer(val id: Int, val key: String, val text: Option[String]) {
  ...
  val responders = new ArrayBuffer[domain.Responder]()

  def clear() {
    responders.clear
  }
  def addResponder(responder: domain.Responder) {
  ...
  def getResponders(): Array[domain.Responder] = {
    var r = new Array[domain.Responder](responders.length)
