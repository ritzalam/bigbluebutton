...
import org.bigbluebutton.core.domain.{ Page, SimplePollOutVO, SimplePollResultOutVO }
...
class MessageSenderActor(val service: MessageSender)
  ...
  private def UserRespondedToPollMessageTpJson(msg: UserRespondedToPollMessage): String = {
    val pollResultVO = pollResultVOtoMap(msg.poll)
    val psm = new org.bigbluebutton.common.messages.UserVotedPollMessage(msg.meetingId.value, msg.requesterId.value, pollResultVO)
    psm.toJson
  }
  ...
  private def handleBroadcastLayoutEvent(msg: BroadcastLayoutEvent) {
    ...
    val evt = new BroadcastLayoutMessage(msg.meetingId.value,
      msg.setByUserId.value, msg.layoutID, msg.locked, users)
    service.send(MessagingConstants.FROM_USERS_CHANNEL, evt.toJson())
  }

  private def handleGetCurrentLayoutReply(msg: GetCurrentLayoutReply) {
    val reply = new GetCurrentLayoutReplyMessage(msg.meetingId.value,
      msg.requesterId.value, msg.setByUserId.value, msg.layoutID, msg.locked)
  ...
  private def handleMuteVoiceUser(msg: MuteVoiceUser) {
    val m = new MuteUserInVoiceConfRequestMessage(msg.meetingId.value,
      msg.voiceConfId.value, msg.voiceUserId.value, msg.mute)
    service.send(MessagingConstants.TO_VOICE_CONF_SYSTEM_CHAN, m.toJson())
  }
